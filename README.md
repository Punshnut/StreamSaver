...
- **Subtitles** / **Low Latency** — mirror Twitch's own player toggles for closed captions and Low Latency playback. Flip one and StreamSaver applies it to the active tab immediately, then keeps it applied on page load and channel navigation (and, with Aggressive Mode on, a periodic re-check about every 60 seconds) so it doesn't get lost to Twitch's own per-stream defaults.
